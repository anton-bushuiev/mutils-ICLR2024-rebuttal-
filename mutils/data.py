...
def load_SKEMPI2(
    ...
    if preprocess:
        ...
        # Drop entries with unfeasible ddG label
        # mask_nolabel = df['Affinity_mut (M)'].isna() |\
        #                df['Affinity_wt (M)'].isna() |\
        #                df['Temperature'].isna()
        # df = df[~mask_nolabel]

    # Calculate binding energies and binding energy changes
    if ddG or dG:
        # TODO Should we include values like "~2.2E-14"
        # TODO What about > vales like ">4E-06"
        # TODO What about < values
        # def convert_less_than_value(val):
        #     if val[0] == '<' and int(val[-3:]) < -6:
        #         return val[1:]
        #     else:
        #         return val
        # df['Affinity_mut (M)'] = df['Affinity_mut (M)'].apply(convert_less_than_value)
        # df['Affinity_wt (M)'] = df['Affinity_mut (M)'].apply(convert_less_than_value)
        # df['Affinity_mut (M)'] = df['Affinity_mut (M)'].replace({'<1E-11': '0.0'})
        # df['Affinity_wt (M)'] = df['Affinity_wt (M)'].replace({'<1E-11': '0.0'})
